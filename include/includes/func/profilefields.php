  ...
  while ( $r = db_fetch_assoc($q)) {
    if ( $r['func'] == 1 ) {
      $str = @db_result ( db_query ("SELECT val FROM prefix_userfields WHERE uid = ".$uid." AND fid = ".$r['id']) , 0 );
			$l .= '<tr><td class="active">'.$r['show'].'</td><td class="success">'.$str.'</td></tr>';
    } elseif ( $r['func'] == 2 ) {
      $l .= '<tr><td class="warning" colspan="2"><b>'.$r['show'].'</b></td></tr>';
    } elseif ( $r['func'] == 3 ) {
      $str = '';
      if (isset($a[$r['show']])) {
      ...
      if ( function_exists ( 'profilefields_show_spez_'.$r['show'] ) ) {
        ...
        $l .= '<tr><td class="active">'.ucfirst($r['show']).'</td><td class="success">'.$str.'</td></tr>';
      }
    }
  }
  return ( $l );
...
function profilefields_show_spez_usergallery ($value,$uid) {
  ...
  if ($allgAr['forum_usergallery'] == 1) {
    return ( profilefields_show_echo_standart ( 'Usergallery', '<a href="index.php?user-usergallery-'.$uid.'"><button type="button" class="btn btn-success">ansehen</button></a>' ) );
  }
}
...
function profilefields_show_spez_opt_mail ($value,$uid) {
  global $lang;
  return ( profilefields_show_echo_standart ( $lang['mail'], ($value?'<a href="index.php?user-mail-'.$uid.'"><button type="button" class="btn btn-success">senden</button></a>':'') ) );
}
function profilefields_show_spez_opt_pm ($value,$uid) {
  global $lang;
  return ( profilefields_show_echo_standart ( $lang['privatemessages'], ($value?'<a href="index.php?forum-privmsg-new=0&amp;empfid='.$uid.'"><button type="button" class="btn btn-success">senden</button></a>':'') ) );
}
...
function profilefields_show_echo_standart ( $k, $v ) {
  return ( '<tr><td class="active">'.$k.'</td><td class="success">'.$v.'</td></tr>' );
}
?>