...
function sendpm ($sid,$eid,$ti,$te,$status = 0) {
	$gid = db_result(db_query("SELECT MAX(`groupid`) FROM prefix_pm"),0) + 1;
	if (is_array($eid)) { 
		$anz = count($eid);
		db_query("INSERT INTO `prefix_pm` (`sid`,`eid`,`time`,`titel`,`txt`,`groupid`,`anz_in_group`,`status`) ".
		"SELECT  ".$sid.",`prefix_user`.`id`,'".time()."','".$ti."','".$te."','".$gid."','".$anz."',".$status." FROM `prefix_user` WHERE `prefix_user`.`id` IN (" . implode(',', $eid) . ")");
	} else {
		$anz = 1;
		db_query("INSERT INTO `prefix_pm` (`sid`,`eid`,`time`,`titel`,`txt`,`groupid`,`anz_in_group`,`status`) VALUES (".$sid.",".$eid.",'".time()."','".$ti."','".$te."','".$gid."','".$anz."',".$status.")");
	} 
}
?>