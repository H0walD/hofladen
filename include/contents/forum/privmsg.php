...
HTML;
	...
	case 'showsend' :
		...
		while ($row = db_fetch_assoc($erg)) {
			...
			if ($anz == $res) {
				$row['NEW'] = $row['gelesen'] == 0 ? '<i class="fa fa-envelope" alt="ungelesen" title="Nachricht wurde noch nicht gelesen" style="cursor:help;"/></i><a href="index.php?forum-privmsg-edit-'.$pmid.'">&nbsp;&nbsp;<i class="fa fa-wrench" name="edit" title="Nachricht bearbeiten" /></i>' : '';
			} else {
				$row['NEW'] = $row['gelesen'] == 0 ? '<i class="fa fa-envelope" alt="ungelesen" title="Nachricht wurde noch nicht gelesen" style="cursor:help;"/></i>' : '';
			}
    ...
    default :
        ...
        $erg = db_query($abf);
        while ($row = db_fetch_assoc($erg)) {
            $class = ($class == 'active' ? 'success' : 'active');
            $row['NEW'] = ($row['NEW'] == 0 ? '<span style="color: red; text-decoration: blink; font-weight: bold; font-style: italic;">neu</span>' : '');
            $row['CLASS'] = $class;
            $row['BET'] = (trim($row['BET']) == '' ? ' -- kein Nachrichtentitel -- ' : $row['BET']);
            $row['time'] = date('d. ',$row['time']).getSDmon($row['time']).date(' y - H:i \U\h\r', $row['time']);
