...
case 2 :
  ...
  AND strlen ($_POST['txt']) <= $allgAr['Gtxtl'] ) {
	...
	sendpm(0, 1, 'Neuer Gästebucheintrag', ''.$name.' hat einen Gästebucheintrag hinterlassen, bitte überprüfen und weitere Maßnahmen einleiten!');


    $_SESSION['klicktime_gbook'] = $dppk_time;
    wd('index.php?gbook','Vielen Dank. Dein Gästebucheintrag wird nach erfolgreicher Prüfung freigeschaltet!');
	} else {
...
case 'show' :
  if ($allgAr['gbook_koms_for_inserts'] == 1) {
    ...
    if ($r['page'] != '') {
      ...
      $r['page'] = ' &nbsp; <a href="'.$r['page'].'" target="_blank"><i class="fa fa-home fa-2x" data-toggle="tooltip" data-placement="top"  title="zur Homepage vom Poster"></i></a>';
		}
		if ($r['mail'] != '') {
	    $r['mail'] = ' &nbsp; <a href="mailto:'.escape_email_to_show($r['mail']).'"><i class="fa fa-envelope fa-2x" data-toggle="tooltip" data-placement="top"  title="Email an Poster"></i></a>';
		}
...
default :
  ...
  $ei1 = @db_query("SELECT COUNT(ID) FROM prefix_gbook WHERE public = '1'");
  $ein    = @db_result($ei1,0);

	$ar = array ('EINTRAGE' => $ein );
	$tpl->set_ar_out($ar,0);

	$erg = db_query("SELECT * FROM prefix_gbook WHERE public = '1' ORDER BY time DESC LIMIT ".$anfang.",".$limit) or die (db_error());
    ...
    $mail = '';
      ...
      $page = ' &nbsp; <a href="'.$row->page.'" target="_blank"><i class="fa fa-home fa-2x" data-toggle="tooltip" data-placement="top"  title=" zur Homepage vom Poster"></i></a>';
		}
		if ($row->mail) {
	    $mail = ' &nbsp; <a href="mailto:'.escape_email_to_show($row->mail).'"><i class="fa fa-envelope fa-2x" data-toggle="tooltip" data-placement="top"  title="Email an Poster"></i></a>';
		}
    $koms = '';
    if ($allgAr['gbook_koms_for_inserts'] == 1) {
      $koms = db_result(db_query("SELECT COUNT(*) FROM prefix_koms WHERE uid = ".$row->id." AND cat = 'GBOOK'"),0,0);
      $koms = '<div class="col-lg-4 col-md-4 col-xs-12 pull-left"><a class="btn btn-default col-lg-12 col-md-12 col-xs-12" href="index.php?gbook-show-'.$row->id.'"><b class="pull-left"><i class="fa fa-comments"></i>&nbsp;&nbsp;&nbsp;'.$lang['comments'].'</b><span class="badge pull-right" style="font-weight:lighter!important;">'.$koms.'</span></a></div>';
    }
