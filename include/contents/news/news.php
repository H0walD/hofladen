...
function news_find_kat ($kat) {
    ...
    if (!empty($pfadzumBild) ) {
        $kategorie = $pfadzumBild;
    } else {
        $kategorie = 'include/images/news/Allg.png';
    }
...
if (!is_numeric($menu->get(1))) {
    ...
    ORDER BY news_time DESC LIMIT 15";
        ...
        if (db_num_rows($tn_sql) > 0) {
        	...
        	if (!$dontShow) {
				...
				$tn_r['text'] = substr($tn_r['news_text'], 0, 255);
        	    $tn_r['text'] = $tn_r['html']? $tn_r['news_text']: bbcode('test'. $tn_r['news_text']);
    ...
    LIMIT " . $anfang . "," . $limit;
        ...
        while ($row = db_fetch_assoc($erg)) {
            ...
            $row['datum'] = $row['datum']; // $lang[$row['dayofweek']] . ' ' . 
            ...
            } elseif( strlen($row['text']) > 255 ) {
				$row['html'] = true;
				$row['text'] = substr($row['text'], 0, 255).'...';
				$row['text'] .= '<div class="btn-group btn-group-justified"><a class="btn btn-default" href="index.php?news-' . $row['id'] . '">alles Lesen</a></div>';
                $row['readwholenews'] = '';
            } else {
				$row['text'] = $row['text'];
                $row['readwholenews'] = '';
