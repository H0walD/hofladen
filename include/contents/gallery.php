...
function random_image($id){
	$images = array();
	$res = db_query('SELECT id, endung FROM prefix_gallery_imgs WHERE cat='. $id);
	if ( db_num_rows($res) > 0 ) {
		while( $row = db_fetch_object( $res ) ) {
			$images[] = 'include/images/gallery/img_b3k_'.$row->id.'.'.$row->endung;
		}
		
		return $images[rand(0, count($images)-1)];
	} else {
		return 'http://nigg.howald-design.ch/include/hpbilder/geschichte/1979.png';
	}
}
...
# normale groesse anzeigen
  ...
  if ($allgAr['gallery_img_koms'] == 1) {
	...
	while($row = db_fetch_assoc($erg) ) {
        ...
        if ( $i <> 0 AND ($i % $img_per_line ) == 0 ) { echo '</div><div class="row">'; }
