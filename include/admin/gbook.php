...
if(!empty ($_GET['pub'])){
    $pubid = escape($_GET['pub'], 'integer');
    db_query('UPDATE prefix_gbook SET public = IF(public>0,0,1) WHERE id = "'.$pubid.'" LIMIT 1');
}
...
$class = '';
$erg = db_query('SELECT name, mail, txt, id, public FROM `prefix_gbook` ORDER BY time DESC');
while ($r = db_fetch_assoc($erg) ) {
  $class = ($class == 'Cmite' ? 'Cnorm' : 'Cmite' );
  $pub = '<a href="admin.php?gbook&pub='.$r['id'].'">';
    if($r['public'] == 1){
        $pub .= '<img src="include/images/icons/jep.gif" title="Eintrag ist freigeschaltet!" alt="Eintrag ist freigeschaltet!" border="0" /></a>';
    }else{
        $pub .= '<img src="include/images/icons/nop.gif" title="Eintrag ist gesperrt!" alt="Eintrag ist gesperrt!" border="0" /></a>';
    } 
  ...
  echo '<tr class="'.$class.'">';
  echo '<td>'.$pub.'&nbsp;<a href="admin.php?gbook=0&edit='.$r['id'].'">';
