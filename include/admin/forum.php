...
if ( $show ) {
  ...
  while ($row = db_fetch_assoc($erg) ) {
    $class = ($class == 'Cmite' ? 'Cnorm' : 'Cmite' );
      ...
      case when view  <= 0 then vg.name else vg.name end as view,
      case when reply <= 0 then rg.name else rg.name end as reply,
      case when start <= 0 then sg.name else sg.name end as start
    FROM prefix_forums
