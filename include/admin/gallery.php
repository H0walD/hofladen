...
function ThumbnailsFromMiddle( $file ,$nfile, $nbreit, $nhoehe){
    ## BILD EIGENSCHAFTEN
    list( $breit, $hoehe, $type, $leer) = getimagesize( $file ); #1 = GIF, 2 = JPG, 3 = PNG, 4 = SWF
    #$neueHoehe=intval($hoehe*500/$breit);
    ## Berechnug von X und Y für das Kopieren des Original Bild.
    $b = ceil((($breit - $nbreit) / 2));
    $h = ceil((($hoehe - $nhoehe) / 2));
    ## Neues Bild erstellen und Weiß färben.
    if ( function_exists ( 'imagecreatetruecolor' ) ){
        $npic = imagecreatetruecolor($nbreit,$nhoehe);
    }else{
        $npic = imagecreate($nbreit,$nhoehe);
    }
    $cw = imagecolorallocate($npic,255,255,255);
    ## Bild Type bestimmen und neue erstellen.
    switch( $type ){
        case '1':
            ### GIF ##############################################
            $apic = imagecreatefromgif($file);
            imagecopy($npic,$apic,0,0,$b,$h,$nbreit,$nhoehe);
            imagegif($npic,$nfile);
        break;
        case '2':
            ### JPEG #############################################
            $apic = imagecreatefromjpeg($file);
            imagecopy($npic,$apic,0,0,$b,$h,$nbreit,$nhoehe);
            imagejpeg($npic,$nfile);
        break;
        case '3':
            ### PNG ##############################################
            $apic = imagecreatefrompng($file);
            imagecopy($npic,$apic,0,0,$b,$h,$nbreit,$nhoehe);
            imagepng($npic,$nfile);
        break;
    }
...
if ( $menu->getA(1) == 'D' ) {
    ...
    if ($menu->get(2) == 'delall') {
        ...
        while($r2 = db_fetch_assoc($sql)){
			...
			@unlink ('include/images/gallery/img_b3k_'.$r2['id'].'.'.$r2['endung']);
        }
...
if ( $menu->getA(1) == 'r' ) {
  ...
  if (file_exists ($bild_url)) {
    $bild_thumb = 'include/images/gallery/img_thumb_'.$id.'.'.$endung;
    $bild_norm  = 'include/images/gallery/img_norm_'.$id.'.'.$endung;
	$bild_b3k  = 'include/images/gallery/img_norm_'.$id.'.'.$endung;
    create_thumb ($bild_url, $bild_thumb, $allgAr['gallery_preview_width']);
    create_thumb ($bild_url, $bild_norm , $allgAr['gallery_normal_width']);
	ThumbnailsFromMiddle( $bild_norm ,$bild_b3k, 300, 125);
  }
